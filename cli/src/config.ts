...
export function loadConfig(): Config {
  const paths = [
    resolve(process.cwd(), 'music-cli.toml'),
    resolve(process.cwd(), '..', 'music-cli.toml'),
    resolve(process.env.HOME || '~', '.config', 'music-cli.toml'),
  ];
  ...
  throw new Error(
    `Config file not found. Searched:\n${paths.map(p => `  - ${p}`).join('\n')}\n\nCopy music-cli.toml.example to music-cli.toml and fill in your credentials.`
  );
}
